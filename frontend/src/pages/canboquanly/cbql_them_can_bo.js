...
const CanBoThem = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/danh-sach-can-bo-huong-dan'), 1000);
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="form_grid">
        ...
        <div className="donvi_grid">
          ...
          >
            ...
            {donViList.map(dv => (
              <option key={dv.ma_don_vi} value={dv.ma_don_vi}>{dv.ten_don_vi}</option>
            ))}
          </select>
          ...
          {showList && searchDonVi.trim() !== '' && (
            <div className="donvi_dropdown">
              {donViList
                ...
                .map(dv => (
                  <div key={dv.ma_don_vi} className="donvi_item"
                    onClick={() => {
              ...
              {donViList.filter(dv => dv.ten_don_vi.toLowerCase().includes(searchDonVi.toLowerCase())).length === 0 &&
        ...
        <div className="form_actions">
          ...
          <button type="button" className="cancel_btn" onClick={() => navigate(-1)}>Huỷ</button>
        </div>
      </form>
    </div>
  );
