import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { canBoHuongDanService, donViService } from '../../services/api';
import '../../styles/them_can_bo_huong_dan.css';

const CanBoThemCanBoHuongDan = () => {
  const [canBo, setCanBo] = useState({
    ho_ten: '',
    gioi_tinh: 'Khác',
    so_dien_thoai: '',
    email_can_bo: '',
    so_tk_ngan_hang: '',
    chuc_vu: '',
    chuyen_mon: '',
    ma_don_vi: '',
  });

  const [donViList, setDonViList] = useState([]);

  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');

  const navigate = useNavigate();

  const [searchDonVi, setSearchDonVi] = useState('');
  const [showList, setShowList] = useState(false);


  // Lấy danh sách đơn vị đưa vào dropdown
  useEffect(() => {
    const fetchDonVi = async () => {
      try {
        const res = await donViService.getAll();
        setDonViList(res.data);
      } catch (error) {
        console.error('Lỗi lấy đơn vị:', error);
      }
    };
    fetchDonVi();
  }, []);

  // Xử lý thay đổi input
  const handleChange = (e) => {
    const { name, value } = e.target;
    setCanBo({ ...canBo, [name]: value });
  };

  // Submit form
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await canBoHuongDanService.create(canBo);

      setSuccess(`Tạo cán bộ thành công: ${res.data.ma_can_bo}`);
      setError('');

      setTimeout(() => {
        navigate('/danh-sach-can-bo-huong-dan');
      }, 1000);
    } catch (err) {
      console.error(err);
      if (err.response) setError(err.response.data.message);
      else setError("Thêm cán bộ thất bại");
      setSuccess('');
    }
  };

  return (
    <div className="form_container">
      <h1>Thêm cán bộ hướng dẫn</h1>

      {error && <div className="error-message">{error}</div>}
      {success && <div className="success-message">{success}</div>}

      <form onSubmit={handleSubmit} className="form_grid">

        <label>Họ tên:</label>
        <input
          type="text"
          name="ho_ten"
          value={canBo.ho_ten}
          onChange={handleChange}
          required
        />

        <label>Giới tính:</label>
        <select name="gioi_tinh" value={canBo.gioi_tinh} onChange={handleChange}>
          <option value="Nam">Nam</option>
          <option value="Nữ">Nữ</option>
          <option value="Khác">Khác</option>
        </select>

        <label>Số điện thoại:</label>
        <input
          type="text"
          name="so_dien_thoai"
          value={canBo.so_dien_thoai}
          onChange={handleChange}
        />

        <label>Email:</label>
        <input
          type="email"
          name="email_can_bo"
          value={canBo.email_can_bo}
          onChange={handleChange}
        />

        <label>Số tài khoản ngân hàng:</label>
        <input
          type="text"
          name="so_tk_ngan_hang"
          value={canBo.so_tk_ngan_hang}
          onChange={handleChange}
        />

        <label>Chức vụ:</label>
        <input
          type="text"
          name="chuc_vu"
          value={canBo.chuc_vu}
          onChange={handleChange}
        />

        <label>Chuyên môn:</label>
        <input
          type="text"
          name="chuyen_mon"
          value={canBo.chuyen_mon}
          onChange={handleChange}
        />

        <label>Đơn vị:</label>

<div className="donvi_row" style={{ position: "relative" }}>
  {/* Ô tìm kiếm */}
  <input
    type="text"
    placeholder="Tìm đơn vị..."
    value={searchDonVi}
    onChange={(e) => {
      setSearchDonVi(e.target.value);
      setShowList(true);
    }}
    onFocus={() => setShowList(true)}
    className="input_search_donvi"
  />

  {/* Dropdown select */}
  <select
    name="ma_don_vi"
    value={canBo.ma_don_vi}
    onChange={handleChange}
    className="select_donvi"
  >
    <option value="">-- Chọn đơn vị --</option>
    {donViList.map((dv) => (
      <option key={dv.ma_don_vi} value={dv.ma_don_vi}>
        {dv.ten_don_vi}
      </option>
    ))}
  </select>

  {/* List hiện khi gõ tìm */}
  {showList && searchDonVi.trim() !== '' && (
    <div className="donvi_dropdown">
      {donViList
        .filter((dv) =>
          dv.ten_don_vi.toLowerCase().includes(searchDonVi.toLowerCase())
        )
        .map((dv) => (
          <div
            key={dv.ma_don_vi}
            className="donvi_item"
            onClick={() => {
              setCanBo({ ...canBo, ma_don_vi: dv.ma_don_vi });
              setSearchDonVi(dv.ten_don_vi);
              setShowList(false);
            }}
          >
            {dv.ten_don_vi}
          </div>
        ))}

      {donViList.filter((dv) =>
        dv.ten_don_vi.toLowerCase().includes(searchDonVi.toLowerCase())
      ).length === 0 && (
        <div className="donvi_noresult">Không tìm thấy đơn vị</div>
      )}
    </div>
  )}
</div>




        <button type="submit" className="btn btn-primary">Thêm cán bộ</button>
      </form>
    </div>
  );
};

export default CanBoThemCanBoHuongDan;